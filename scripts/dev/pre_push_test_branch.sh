...
#  git clean -xfd
#fi
git checkout -f lursight/main | true
git checkout -f origin/main | true
git checkout -f "$SHA_UNDER_TEST" || exit 99

# clean any artefacts up and reseting the state to a pure state.
printf "%s reseting to head with hard, and cleaning'\\n" "$LOG_PREFIX"
if [ -z "$NO_CLEAN" ]; then
  git reset --hard head
  git clean -xfd
fi
